...
if(generalInfo.photo!="" && generalInfo.photo!=null){
document.getElementById("profile_img").innerHTML='<img class="profile_photo" src="'+generalInfo.photo+'" alt="profile_photo">';
}else{
...
const A4_HEIGHT_PX = 1122; // A4 height in px at 96dpi (or 297mm)
const A4_WIDTH_PX = 794;   // A4 width in px at 96dpi (or 210mm)
const PAGE_PADDING = 40;   // Internal padding

function splitResumeIntoPages() {
    const originalContent = document.getElementById('cv');
    ...
    let currentHeight = PAGE_PADDING * 2; // top + bottom padding
  
    console.log('Starting pagination...');
    console.log(allChildren);
    
    console.log('Total children to process:', allChildren.length);
    ...
    allChildren.forEach((child, index) => {
      ...
      console.log(`Child ${index} added. Current page height: ${tempHeight}px`);
      console.log(clonedChild);
      console.log(tempHeight+">"+A4_HEIGHT_PX);
      
  
      if (tempHeight > A4_HEIGHT_PX) {
        console.log("entered");
        
        console.warn(`Overflow detected at child ${index}. Height: ${tempHeight}px > ${A4_HEIGHT_PX}px`);
  
        // Remove overflowing child and start a new page
        page.removeChild(clonedChild);
        page = createNewPage();
        container.appendChild(page);
        page.appendChild(clonedChild);
        console.log(`Moved child ${index} to new page`);
      }else{
        ...
        if(company.roles!="" && company.roles!=null){
            company.roles.forEach((role) => {
                ...
                if(role.description!="" && role.description!=null){
                    work_div=work_div+'<div>'+role.description+'</div>';    
                }
...
internships.forEach((internships) => {
    ...
    if(internships.description!="" && internships.description!=null){
        intern_div+='<div> '+internships.description+'</div>';
...
if(education.length>0){
 ...
 skills.forEach((skill) => {
     if(skill.name!="" && skill.name!=null){
        skillHTML=skillHTML+'<li><span>'+skill.name+'</span>';
        if(skill.level>0){
            skillHTML=skillHTML+'<div class="sb-skeleton"><div class="skillbar" style="--pgbar-length: '+skill.level+'%"></div></div></li>';
        }else{
            skillHTML=skillHTML+'</li>';
        }
     }
 ...
 if(volunteeringHTML!=null && volunteeringHTML!=""){
     ...
     softwareInfo.forEach((softwareInfo) => {
         ...
         if(softwareInfo.rating!="" && softwareInfo.rating!=null){
            if(softwareInfo.rating<=0){
                software_div+='</li>';
            } 
            else if(softwareInfo.rating<=5){
    ...
    projects.forEach((projects) => {
        ...
        if(projects.summary!="" && projects.summary!=null){
            projects_div+='<div>'+projects.summary+'</div>';
        }
...
let social_count=0;
    ...
    if(generalInfo.hasOwnProperty("summary") && generalInfo.summary != null){
        document.getElementById("summary").innerHTML='<div>'+generalInfo.summary+'</div>';
    }else{
