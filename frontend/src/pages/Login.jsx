...
const from = location.state?.from || "/"
    ...
    const onSubmitHandler = async (e) => {
        ...
        try {
            ...
            if (state === 'Sign Up') {
                ...
                if (data.success) {
                    setIsLoggedIn(true);
                    navigate(from); // Redirect to the stored path
                } else {
                    toast.error(data.message);
                }
            } else {
    ...
    return (
        <div className='flex items-center justify-center min-h-screen px-6 sm:px-0'>
            <img onClick={() => navigate('/')} src="/resumeRingerLogo.png" alt="" className='absolute left-5 sm:left-20 top-5 w-20 sm:w-20 cursor-pointer' />
            <div className=' p-10 rounded-lg shadow-lg w-full sm:w-96 text-black text-sm bg-gray-300'>
                ...
                <form onSubmit={onSubmitHandler}>
                    ...
                    <button className='w-full py-2.5 rounded-full bg-[linear-gradient(90deg,_hsla(133,_68%,_60%,_1)_0%,_hsla(205,_97%,_42%,_1)_100%)]  px-4 hover:bg-[linear-gradient(90deg,_hsla(205,_97%,_42%,_1)_0%,_hsla(133,_68%,_60%,_1)_100%)] text-black font-medium cursor-pointer'>{state}</button>
                </form>
                {state === "Sign Up" ? (
                    <p className='text-gray-400 text-center text-xs mt-4'>Already have an account ? {' '}
                        <span onClick={() => setState('Login')} className='text-black cursor-pointer underline'>Login here</span>
                    </p>
                ) : (
                    <p className='text-gray-400 text-center text-xs mt-4'>Don't have an account ? {' '}
                        <span onClick={() => setState('Sign Up')} className='text-black cursor-pointer underline'>Sign Up</span>
                    </p>
                )}
            </div>
        </div>
